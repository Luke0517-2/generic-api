...
public class MysqlJdbcAOP {
    ...
    public MysqlJdbcAOP(
            @Qualifier("mysqlConnectionProvider") MultiTenantConnectionProvider<String> connectionProvider,
    ...
    protected Object runAroundAdvice(final ProceedingJoinPoint pjp) throws Throwable {
        ...
        if (tenantId == null) {
            log.info("No tenantId resolved, proceeding without tenant-specific context.");
            return pjp.proceed();
        }

        Connection connection = connectionProvider.getConnection(tenantId);
        try {
            log.info("Tenant-specific connection obtained for tenantId: {}", tenantId);
            return pjp.proceed();
        } finally {
            connectionProvider.releaseConnection(tenantId, connection);
            log.info("Tenant-specific connection released for tenantId: {}", tenantId);
        }
    }
}
